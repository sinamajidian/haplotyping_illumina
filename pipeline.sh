...
mkdir haplotypes
python2 ../software/haplotyping_illumina/haplogenerator/haplogenerator.py -f ref.fasta -o haplotypes/solanum -m "{'A':'C','C':'A','G':'T','T':'G'}"  -p 2 -v --model poisson  -s [.01,0,0]
wc -l haplotypes/hap_varianthaplos.txt

haplotypes/solanum_hap2.fa
############# B1: Sequencing reads
mkdir reads
art_illumina -p -na -i haplotypes/solanum_hap1.fa -l 250 -f 30 -m 300 -s 10 -o reads/reads_hap1_R
art_illumina -p -na -i haplotypes/solanum_hap1.fa -l 250 -f 30 -m 300 -s 10 -o reads/reads_hap2_R
...
cat variants.vcf | grep "0/1" | grep "snp" > variants_het.vcf

# check the number of called the variants and number of introduced snps in haplogenerator output
...
## some stats
samtools depth  filem.bam  |  awk '{sum+=$3} END { print "Average of coverage is ",sum/NR}'
